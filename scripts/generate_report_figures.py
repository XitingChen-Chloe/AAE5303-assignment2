...
Reads TUM trajectories, matches poses by timestamp, applies Sim(3) alignment
from evo_ape --save_results zip, and produces a 2x2 report figure:
1) 2D trajectory before alignment
2) 2D trajectory after Sim(3) alignment
3) ATE error histogram
...
def _associate_by_time(t_gt: np.ndarray, t_est: np.ndarray, t_max_diff_s: float) -> Tuple[np.ndarray, np.ndarray]:
    """Greedy two-pointer association (monotonic timestamps). Returns (gt_indices, est_indices)."""
    gt_idx: List[int] = []
    est_idx: List[int] = []
    i, j = 0, 0
